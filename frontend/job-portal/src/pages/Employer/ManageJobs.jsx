...
import { ChevronDown, ChevronUp, Edit, Plus, PlusIcon, Search, Trash2, Users, X } from 'lucide-react';
...
const ManageJobs = () => {
  ...
  const filteredAndSortedJobs = useMemo(() => {
    let filtered = jobs.filter((job) => {
      const matchesSearch = 
        ...
        job.company.includes(searchTerm.toLowerCase())
      const matchesStatus = 
  ...
  // toggle status of specific job 
  const handleStatusChange = async (jobId) => {
    try {
      const response = await axiosInstance.put(
        API_PATHS.JOBS.TOGGLE_CLOSED(jobId)
      );
      getPostedJobs(true);
    } catch (error) {
      console.error('Error togglinf job status: ', error)
    }
  }
    ...
    <DashboardLayout activeMenu='manage-jobs'>
      <div className='min-h-screen p-4 sm:p-6 lg:p-6'>
        <div className='max-w-7xl mx-auto'>
          ...
          {/* filter */}
          <div className='bg-white/80 backdrop-blur-sm rounded-2xl shadow-black/5 border border-white/20 p-6 md-6'>
            ...
            <div className='bg-white/80 backdrop-blur-sm rounded-2xl border border-white/20 overflow-hidden'>
              {filteredAndSortedJobs.length === 0 && !isLoading ?
                ...
                :
                (
                  <div className='w-[75vw] md:w-full overflow-x-auto scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-transparent hover:scrollbar-thumb-gray-300'>
                      ...
                      <tbody className='bg-white divide-y divide-gray-200'>
                        {isLoading ? 
                          ...
                          paginatedJobs.map((job) => {
                            return (
                              <tr 
                                className='hover:bg-blue-50/30 transition-all duration-200 border-b border-gray-100/50'
                                key={job.id}
                              >
                                <td className='px-6 py-5 whitespace-nowrap- min-w-[200px] sm:min-w-8'>
                                  <div>
                                ...
                                <td className='px-6 py-5 whitespace-nowrap min-w-[130px] sm:min-w-0'>
                                  <button
                                    className='flex items-center text-sm text-blue-600 hover:text-blue-600 font-semibold transition-colors duration-200 hover:bg-blue-50 px-2 py-1 rounded-lg'
                                ...
                                <td className='px-6 py-5 whitespace-nowrap text-sm font-medium min-w-[130px] sm:min-w-0'>
                                  <div className='flex space-x-2'>
                                    <button 
                                      className='text-blue-400 hover:text-blue-800 p-2 rounded-lg hover:bg-blue-50 transition-colors duration-200'
                                      onClick={() => 
                                        navigate('/post-job', {
                                          state: { jobId : job.id } 
                                        }
            ...
            {totalPages > 1 && (
              <div className='mt-6 flex items-center justify-between'>
                <div className='flex-1 flex justify-between sm:hidden'>
                  <button
                    onClick={() => setCurrentPage(Math.ceil(1, currentPage - 1))}
                  ...
                  <button
                    onClick={() => 
                      setCurrentPage(Math.ceil(totalPages, currentPage + 1))
                    }
                ...
                <div className='hidden sm:flex-1 sm:flex sm:items-center sm:justify-between'>
                  ...
                  <div>
                    <nav className='showingrelative z-0 inline-flex rounded-md shadow-sm -space-x-px'>
                      <button
                      ...
                      {Array.from({ length: totalPages }, ( _, l) => 1 + l).map(
                        (page) => (
                          <button
                            key={page}
                      ...
                      <button
                        ...
                        disabled={currentPage === totalPages}
                        className='relative inline-flex items-center px-2 py-2 rounded-r-md border border-gray-300 bg-white text-sm font-medium text-gray-500 hover:bg-green-50 disabled:opacity-gray-50 disabled:cursor-not-allowed'
                      >
                        Next 
                      </button>
